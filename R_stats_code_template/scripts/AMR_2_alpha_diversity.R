...
AMR_alpha_div <- estimate_richness(data_noSNP, measures = c("Observed","Shannon","Simpson","InvSimpson"))
AMR_alpha_div
AMR_alpha_div.df <- as(sample_data(data_noSNP), "data.frame")
AMR_alpha_div_meta <- cbind(AMR_alpha_div, AMR_alpha_div.df)


ggplot(AMR_alpha_div_meta, aes(x= sample_type, y= Observed, fill = sample_type, colour = sample_type)) +
  theme_bw() + 
...
ggplot(AMR_alpha_div_meta, aes(x= sample_type, y= Shannon, fill = sample_type, colour = sample_type)) +
  theme_bw() + 
...
alpha_div_fecal <- alpha_div_meta[which(alpha_div_meta$sample_type=="Fecal"),]
