...
#trial_SNV_data_noSNP <- subset_samples(SNV_data_noSNP, Trial!="")
...
rel_abund <- transform_sample_counts(SNV_data_noSNP.css, function(x) {x/sum(x)}*100)

# agglomerate
ra_class <- tax_glom(rel_abund, taxrank = "class")
ra_class # 50 classes, 216 samples


ra_class_melt_temp <- psmelt(ra_class)


ps_class.css <- tax_glom(SNV_data_noSNP.css, taxrank = "class")
ps_class.melt <- psmelt(ps_class.css)

ra_mechanism <- tax_glom(rel_abund, taxrank = "mechanism")
ra_mechanism # 154 mechanisms
...
ra_class_melt_temp[ra_class_melt_temp$class %in% remainder,]$class <- 'Low abundance variant (<%)'
...
data.adonis <- adonis2(data.dist ~ sample_type, data = SNV_data_noSNP.css.df, permutations = 9999)
...
#This wasn't working. No idea what I did, but it works now, with no NAs
...
ps_AMR.dist <- vegdist(t(otu_table(SNV_data_noSNP)), method = "bray")
...
gene_pos_table_ps_AMR$trt <-  factor(gene_pos_table_ps_AMR$trt, levels = c("Conventional Beef","RWA Beef","Start_CONV", "Midpoint_CONV", 
                                                                                 "End_CONV","Start_RWA", "Midpoint_RWA","End_RWA","Final_washout"))
...
#MM changed Combined order to Trial
ps_group.css <- tax_glom(SNV_data_noSNP.css, taxrank = "group")
ps_group.css <- subset_samples(ps_group.css , Trial != "")
