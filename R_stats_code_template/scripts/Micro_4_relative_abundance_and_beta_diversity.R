...
rel_abund <- transform_sample_counts(data_micro.ps.css, function(x) {x/sum(x)}*100)

# agglomerate
ra_phylum <- tax_glom(rel_abund, taxrank = "Phylum")
ra_phylum # 20 phylum, 153 samples


ra_phylum_melt_temp <- psmelt(ra_phylum)


ps_phylum.css <- tax_glom(data_micro.ps.css, taxrank = "Phylum")
ps_phylum.melt <- psmelt(ps_phylum.css)
...
ra_phylum_melt_temp$Phylum <- as.character(ra_phylum_melt_temp$Phylum)

# group dataframe by phylum, calculate median rel. abundance
medians <- ddply(ra_phylum_melt_temp, ~Phylum, function(x) c(median=median(x$Abundance)))
...
ra_phylum_melt_temp[ra_phylum_melt_temp$Phylum %in% remainder,]$Phylum <- 'Low abundance Phyla (<1%)'

# Determine the number of taxa
length(unique(ra_phylum_melt_temp$Phylum))
...
data.adonis <- adonis2(data.dist ~ sample_type, data = data_micro.ps.css.df, permutations = 9999)
...
sample_data(ps_class.css)$Trial <- factor(sample_data(ps_class.css)$Trial, levels = c("Conventional Beef","RWA Beef","Group 1", "Group 2"))
...
ps_AMR.dist <- vegdist(t(otu_table(data_micro.ps)), method = "bray")
...
# Add metadata
AMR_mapfile$SampleID <- row.names(AMR_mapfile)
ps_AMR.dendro.data$labels <- left_join(ps_AMR.dendro.data$labels, AMR_mapfile, by = c("label" = "SampleID"))
...
gene_pos_table_ps_AMR <- with(
  ...
  data.frame(y_center = x, gene = as.character(label), x = y , trt = as.character(Combined_order), treatment_order = as.character(Combined_order) , height = 1))
...
plt_dendr_class_ps_class <- ggplot(segment_data_ps_AMR) + 
  ...
  scale_x_reverse() + 
  scale_color_manual(values=c("#ff0000", "#cc0000", "#990000", "#0000ff", "#0000cc", "#000099", "#808080", "#ff6347", "#4169e1")) +
...
# This requires a directory called figures to be in your working directory
# All 3 lines have to be run for the file to be created.
png("Figures/SNV_AMR_Class_AbunDendro_bysampletype_fig.png", width = 1200, height = 800)
class_ps_class_plots
dev.off()
...
ps_group.css <- tax_glom(data_micro.ps.css, taxrank = "Group")
