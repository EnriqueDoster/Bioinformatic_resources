...
# Change the right hand part of the formula
  tittle = "Castration groups at Weaning: Phylum-level pairwise comparison"

  da = "At_weaning"
  vari = "castration"
  relevel = c("Birth vs Not castrated", "Pre-weaning vs Birth", "Pre-weaning vs Not castrated", "Pre-weaning vs Turn-out", "Turn-out vs Birth", "Turn-out vs Not castrated")
...
#8. EXTRA last-minute added
```{r}

 # FIGURES FOR DEFENSE
 
# Barplot for sequencing comparison
 
 # Create sample data
 SMS <- c("CS = 0", "CS = 1")
 Low_quality_and_host_reads <- c(12.2,12.2)
 Unclassified_reads <- c(36.13,41.91)
 Classified_reads <- c(5.83,0.05)
 
 # Combine data into a data frame
 df <- data.frame(SMS, Low_quality_and_host_reads, Unclassified_reads, Classified_reads)
 
 
 # Convert data frame from wide to long format
 df_long <- df %>%
   tidyr::pivot_longer(cols = starts_with("group"), names_to = "Group", values_to = "Value")
 ...
 # create dataframe
 category <- as.factor(c("CS = 0","CS = 0","CS = 0", "CS = 1","CS = 1","CS = 1"))
 Group <- as.factor(c("Low-quality & host reads", "Unclassified reads", "Classified reads","Low-quality & host reads", "Unclassified reads", "Classified reads"))
 Value <- c(12.2, 36.13, 5.83,12.2,41.91,0.05)
 Proportion <- c(22.5,66.7,10.8,22.5,77.4,0.1)
 
 df <- data.frame(category, Group, Value, Proportion)
 df$Group <- factor(df$Group, levels = c("Low-quality & host reads", "Unclassified reads", "Classified reads"))
 
 # Create stacked bar plot with total count, group name, and proportion in the legend
 ggplot(df, aes(x = category, y = Value, fill = Group, label = Proportion)) +
   geom_bar(stat = "identity") +
   scale_fill_grey(start = 0.7, end = 0.1) +
   geom_text(position = position_stack(vjust = 0.5), color = "white", size = 4) +
   scale_y_continuous(breaks = c(0, 10, 20, 30, 40, 50))+
   labs(x = "SMS", y = expression("Sequencing reads (x 10)" ^ "6"), fill = "Sequencing reads") +
   ggtitle("No. (%) of sequencing reads in SMS analysis") +
   theme_minimal()

# How accurate is the taxonomic representation of rumen microbiome?
 # create dataframe
 category <- as.factor(c("Standard","Standard","Standard", "Standard","Improved","Improved","Improved","Improved"))
 Group <- as.factor(c("Low-quality reads", "Host reads", "Unclassified reads", "Classified reads","Low-quality reads", "Host reads", "Unclassified reads", "Classified reads"))
 Value <- c(1.9,10.4,36,5.9,1.9,14,30.3,8)
 Proportion <- c(3.5,19.2,66.4,10.9,3.5,25.8,55.9,14.8)
 
 df <- data.frame(category, Group, Value, Proportion)
 df$Group <- factor(df$Group, levels = c("Low-quality reads", "Host reads", "Unclassified reads", "Classified reads"))
 df$category <- factor(df$category, levels=c("Standard","Improved"))
 # Create stacked bar plot with total count, group name, and proportion in the legend
 ggplot(df, aes(x = category, y = Value, fill = Group, label = Proportion)) +
   geom_bar(stat = "identity") +
   ...
   geom_text(position = position_stack(vjust = 0.5), color = "white", size = 4) +
   scale_y_continuous(breaks = c(0, 10, 20, 30, 40, 50))+
   labs(x = "SMS protocol", y = expression("Sequencing reads (x 10)" ^ "6"), fill = "Sequencing reads") +
   ggtitle("No. (%) of sequencing reads in\
classification confidence score = 0") +
 ...
 category <- as.factor(c("Standard","Standard","Standard", "Standard","Improved","Improved","Improved","Improved"))
 Group <- as.factor(c("Low-quality reads", "Host reads", "Unclassified reads", "Classified reads","Low-quality reads", "Host reads", "Unclassified reads", "Classified reads"))
 Value <- c(1.9,10.4,40.6,1.3,1.9,14,36.7,1.6)
 Proportion <- c(3.5,19.2,74.9,2.4,3.5,25.8,67.7,3)
...
classification confidence score = 0.1") +
 ...
 # create dataframe
 Group <- as.factor(c("Low-quality & noise reads", "Chimera reads", "Classified reads"))
 Value <- c(34.37, 0.3, 61.37)
 Proportion <- c(35.79,0.31,63.9)
 
 df <- data.frame(Group, Value, Proportion)
 df$Group <- factor(df$Group, levels = c("Low-quality & noise reads", "Chimera reads", "Classified reads"))
 
 # Create stacked bar plot with total count, group name, and proportion in the legend
 ggplot(df, aes(x="identity",y = Value, fill = Group, label = Proportion)) +
   geom_bar(stat = "identity") +
   scale_fill_grey(start = 0.7, end = 0.1) +
   geom_text(position = position_stack(vjust = 0.5), color = "white", size = 4) +
   scale_y_continuous(breaks = c(0, 20, 40, 60, 80, 100))+
   labs(x = "16S-V4", y = expression("Sequencing reads (x 10)" ^ "3"), fill = "Sequencing reads") +
   ggtitle("No. (%) of sequencing reads in 16S analysis") +
   theme_minimal()+theme(axis.text.x = element_blank())
 
 # OVeraall shotgun reads
 Group <- as.factor(c("Low-quality reads", "Host reads", "Unclassified reads", "Classified reads"))
 Value <- c(1.9,14,36.7,1.6)
 Proportion <- c(3.5,25.8,67.7,3)
 df <- data.frame(Group, Value, Proportion)
 df$Group <- factor(df$Group, levels = c("Low-quality reads", "Host reads", "Unclassified reads", "Classified reads"))
 ...
 ggplot(df, aes(x="identity",y = Value, fill = Group, label = Proportion)) +
   ...
   geom_text(position = position_stack(vjust = 0.5), color = "white", size = 4) +
   scale_y_continuous(breaks = c(0, 10, 20, 30, 40, 50))+
   labs(x = "Rumen SMS analysis", y = expression("Sequencing reads (x 10)" ^ "6"), fill = "Sequencing reads") +
   ggtitle("No. (%) of Rumen Shotgun Metagenomic Sequencing Reads") +
   theme_minimal()+theme(axis.text.x = element_blank())
 
 # By kingdom
 Group <- as.factor(c("Protozoa (0.001%): 40 species", "Virus (0.006%): 390 species" ,"Fungi (0.017%): 83 species", "Archaea (1.37%): 351 species", "Bacteria (98.6%): 8674 species"))
 Proportion <- c(0.001,0.006,0.017,1.37,98.6)
 df <- data.frame(Group, Proportion)
 df$Group <- factor(df$Group, levels = c("Protozoa (0.001%): 40 species", "Virus (0.006%): 390 species" ,"Fungi (0.017%): 83 species", "Archaea (1.37%): 351 species", "Bacteria (98.6%): 8674 species"))
 
 ggplot(df, aes(x="identity",y = Proportion, fill = Group, label = Proportion)) +
   geom_bar(stat = "identity") +
   #geom_text(position = position_stack(vjust = 0.5), color = "white", size = 4) +
   scale_y_continuous(breaks = c(0, 20, 40, 60, 80, 100))+
   ggthemes::scale_fill_tableau("Tableau 20")+
    labs(x = "Rumen microbiome", y = expression("Proportion of total reads"), fill = "Domain/Kingdom") +
   ggtitle("Proportion of classified sequencing \ 
reads by Domain/kingdom") +
   theme_minimal()+theme(axis.text.x = element_blank())
 
# WEIGHTS
 
  df<- read_excel("/Users/gerardodiaz/Desktop/Gerardo/UMN/MS/Thesis/Results/METADATA_ver06.xlsx", sheet = "Weights")
 df$group1 <- factor(df$group1, levels = c("Birth", "Turnout", "Pre-weaning", "Weaning"))
 df$group2 <- factor(df$group2, levels = c("Fence-line", "Truck"))
 df$time <- factor(df$time, levels = c("birth", "turnout", "pre-weaning", "weaning","post-weaning"))
 
 # Calculate mean and standard deviation of weight for each group and time point
 df_summary <- df %>%
   group_by(time, group2) %>%
   summarise(mean_weight = mean(weight),
             sd_weight = sd(weight))
 
 # Create growth curve plot with mean and standard deviation
 ggplot(df_summary, aes(x = time, y = mean_weight, color = group1, group = group1)) +
   geom_line(size=1.5) +
   geom_point(size=3) +
   scale_color_manual(values = c("#f54545", "#85a9ca", "#e0b54e", "#73a054"))+
      geom_errorbar(aes(ymin = mean_weight - sd_weight, ymax = mean_weight + sd_weight),
                 width = 0.2) +
   labs(x = "Collection day", y = "Weight (kg)", color = "Castration group") +
   ggtitle("Growth curve stratified by castration group") +
   theme_minimal() +
   theme(plot.title = element_text(size=17),legend.text = element_text(size = 15), axis.text.x = element_text(angle = 45, hjust = 1, size = 13))
 
 
# ADG comparison
 df<- read_excel("/Users/gerardodiaz/Desktop/Gerardo/UMN/MS/Thesis/Results/METADATA_ver06.xlsx", sheet = "ADG")
 df$castration <- factor(df$castration, levels = c("Birth", "Turnout", "Pre-weaning", "Weaning"))
 df$weaning <- factor(df$weaning, levels = c("Fence-line", "Truck"))
 
 ggplot(df, aes(x = castration, y= weight_ADG, fill= castration))+
   geom_point(position = position_jitter(width = 0.2), alpha = 1, size=3)+
   geom_boxplot(position=position_dodge(0.8),lwd=1, alpha= 0.7)+
   scale_fill_manual(values = c("#f54545", "#85a9ca", "#e0b54e", "#73a054"))+
      theme_classic()+ 
   theme(axis.text.x = element_blank())+
   labs(title= "ADG stratified by castration group",x = "", y = "ADG (kg)", fill = "Castration group") 
   
 ggplot(df, aes(x = weaning, y= weight_ADG, fill= weaning))+
   geom_point(position = position_jitter(width = 0.2), alpha = 1, size=3)+
   geom_boxplot(position=position_dodge(0.8),lwd=1, alpha= 0.7)+
   scale_fill_manual(values = c("#967bba", "#505359"))+
   theme_classic()+ 
   theme(axis.text.x = element_blank())+
   labs(title= "ADG (kg) stratified by weaning group",x = "", y = "ADG (kg)", fill = "Weaning group") 
 
 ANOVA<-aov(df$weight_birth_kg~df$weaning)
 summary(ANOVA)  
 ANOVA<-aov(df$weight_final~df$weaning)
 summary(ANOVA) 
 ANOVA<-aov(df$weight_ADG~df$weaning)
 summary(ANOVA) # THE ONLY SIGNIFICANT
 
 ANOVA<-aov(df$weight_birth_kg~df$castration)
 summary(ANOVA) 
 ANOVA<-aov(df$weight_final~df$castration)
 summary(ANOVA)
 ANOVA<-aov(df$weight_ADG~df$castration)
 summary(ANOVA)

# Non of them significant 
 m <- lm(weight ~ castration + age_days + weaning, data=samples_D3)
 summary(m)
 m <- lm(weight_ADGUpToDate ~ weaning_group + age_days + castration_group, data=samples_D3) # most important model
 summary(m) 
  emmeans(m, pairwise~castration_group)

 m <- lm(weight_birth ~ weaning + age_days + castration, data=samples_D3)
 summary(m) 
 m <- lm(weight_gainedUpToDate ~ weaning + age_days + castration, data=samples_D3)
 summary(m) 
 
 m <- lmer(weight ~ group2*time +group1 + (1|cow_ID), data=df, REML = T)
 summary(m)
 emmeans(m, pairwise~group2|time)

# Differential Abundance Bubble plot of CASTRATION EFFECT AT WEANING
